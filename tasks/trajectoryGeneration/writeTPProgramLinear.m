...
linDiff=computeLinearDifferences(keptJoints,robot,base,ee,toolframe);
...
for i=1:length(multipliers)
    ...
    for n=1:length(indices)
        ...
        if n==1
            ...
            fprintf(fidM,'   %d:L P[%d] %dmm/sec CNT%s   ;\r\n',n+3,n-1,max(round(currentMult*linDiff(n-1)/timeDiff(n-1),0),1), cntTag);
        end
    end
